...
namespace PDCoreNew.Utils
{
    public static class SqlUtils
    {
        ...
        public static Expression<Func<T, bool>> GetFindByDateExpression<T>(string dateF, string dateT, Expression<Func<T, DateTime>> dateSelector)
        {
            ...
            {
                _ = DateTime.TryParse(dateF, out DateTime dateFrom);

                _ = DateTime.TryParse(dateT, out DateTime dateTo);
        ...
        public static string GetTableName(string query)
        {
             ...
             */
            Regex regex = new(@"(?i)from\s+(?<table>((\[.+\](?=\s+(as|where|inner|left|right|join|union)))|\[.+\]\s*$|\S*))");
