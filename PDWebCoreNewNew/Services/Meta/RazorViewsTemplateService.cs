...
namespace PDWebCoreNewNew.Services.Meta
{
    public class RazorViewsTemplateService : ITemplateService
    {
        ...
        public async Task<string> RenderAsync<TViewModel>(string filename, TViewModel viewModel)
        {
            ...
            {
                throw new KeyNotFoundException($"Could not render the HTML, because {filename} template does not exist");
            }
            ...
            try
            {
                ...
                var viewContext = new ViewContext(actionContext, view, viewDictionary, tempDataDictionary, outputWriter, new HtmlHelperOptions());
